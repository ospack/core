module.exports = async ({github, context, core}, formulae_detect, dependent_testing) => {
    ...
    if (label_names.includes(`CI-linux-self-hosted${deps_suffix}`)) {
      ...
      linux_runner = 'ospack-large-bottle-build'
    }
    ...
    if (label_names.includes('CI-skip-dependents')) {
      console.log('CI-skip-dependents label found. Skipping ospack test-bot --only-formulae-dependents.')
      core.setOutput('test-dependents', false)
    } else if (!formulae_detect.testing_formulae) {
      console.log('No testing formulae found. Skipping ospack test-bot --only-formulae-dependents.')
      core.setOutput('test-dependents', false)
    } else {
      console.log('No CI-skip-dependents label found. Running ospack test-bot --only-formulae-dependents.')
    ...
    if (label_names.includes(`CI-test-bot-fail-fast${deps_suffix}`)) {
      console.log(`CI-test-bot-fail-fast${deps_suffix} label found. Passing --fail-fast to ospack test-bot.`)
      test_bot_formulae_args.push('--fail-fast')
      test_bot_dependents_args.push('--fail-fast')
    } else {
      console.log(`No CI-test-bot-fail-fast${deps_suffix} label found. Not passing --fail-fast to ospack test-bot.`)
    }

    if (label_names.includes('CI-build-dependents-from-source')) {
      console.log('CI-build-dependents-from-source label found. Passing --build-dependents-from-source to ospack test-bot.')
      test_bot_dependents_args.push('--build-dependents-from-source')
    } else {
      console.log('No CI-build-dependents-from-source label found. Not passing --build-dependents-from-source to ospack test-bot.')
    }

    if (label_names.includes('CI-skip-recursive-dependents')) {
      console.log('CI-skip-recursive-dependents label found. Passing --skip-recursive-dependents to ospack test-bot.')
      test_bot_dependents_args.push('--skip-recursive-dependents')
    } else {
      console.log('No CI-skip-recursive-dependents label found. Not passing --skip-recursive-dependents to ospack test-bot.')
    }

    if (label_names.includes('CI-skip-livecheck')) {
      console.log('CI-skip-livecheck label found. Passing --skip-livecheck to ospack test-bot.')
      test_bot_formulae_args.push('--skip-livecheck')
    } else {
      console.log('No CI-skip-livecheck label found. Not passing --skip-livecheck to ospack test-bot.')
    }

    if (label_names.includes('CI-version-downgrade')) {
      console.log('CI-version-downgrade label found. Passing --skip-stable-version-audit to ospack test-bot.')
      test_bot_formulae_args.push('--skip-stable-version-audit')
    } else {
      console.log('No CI-version-downgrade label found. Not passing --skip-stable-version-audit to ospack test-bot.')
    }

    if (label_names.includes('CI-checksum-change-confirmed')) {
      console.log('CI-checksum-change-confirmed label found. Passing --skip-checksum-only-audit to ospack test-bot.')
      test_bot_formulae_args.push('--skip-checksum-only-audit')
    } else {
      console.log('No CI-checksum-change-confirmed label found. Not passing --skip-checksum-only-audit to ospack test-bot.')
    }

    if (label_names.includes('CI-skip-revision-audit')) {
      console.log('CI-skip-revision-audit label found. Passing --skip-revision-audit to ospack test-bot.')
      test_bot_formulae_args.push('--skip-revision-audit')
    } else {
      console.log('No CI-skip-revision-audit label found. Not passing --skip-revision-audit to ospack test-bot.')
    }

    if (label_names.includes('CI-skip-new-formulae')) {
      console.log('CI-skip-new-formulae label found. Passing --skip-new to ospack test-bot.')
      test_bot_formulae_args.push('--skip-new')
    } else {
      console.log('No CI-skip-new-formulae label found. Not passing --skip-new to ospack test-bot.')
    }

    if (label_names.includes('CI-skip-new-formulae-strict')) {
      console.log('CI-skip-new-formulae-strict label found. Passing --skip-new-strict to ospack test-bot.')
      test_bot_formulae_args.push('--skip-new-strict')
    } else {
      console.log('No CI-skip-new-formulae-strict label found. Not passing --skip-new-strict to ospack test-bot.')
    }
