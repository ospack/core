class Libspatialite < Formula
  ...
  def install
    ...
    # Ensure Ospack's libsqlite is found before the system version.
    sqlite = Formula["sqlite"]
    ENV.append "LDFLAGS", "-L#{sqlite.opt_lib}"
    ENV.append "CFLAGS", "-I#{sqlite.opt_include}"

    args = %W[
      --with-sysroot=#{OSPACK_PREFIX}
      --enable-geocallbacks
      --enable-rttopo=yes
    ]
  ...
  test do
    # Verify mod_spatialite extension can be loaded using Ospack's SQLite
