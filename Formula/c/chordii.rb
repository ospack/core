class Chordii < Formula
  ...
  test do
    (testpath/"ospack.cho").write <<~EOS
      {title:Ospack}
      {subtitle:I can't write lyrics. Send help}

      [C]Thank [G]You [F]Everyone,
      [C]We couldn't maintain [F]Ospack without you,
      [G]Here's an appreciative song from us to [C]you.
    EOS

    system bin/"chordii", "--output=#{testpath}/ospack.ps", "ospack.cho"
    assert_predicate testpath/"ospack.ps", :exist?
  end
end
