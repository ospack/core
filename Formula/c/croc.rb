class Croc < Formula
  ...
  test do
    ...
    fork do
      exec bin/"croc", "--relay=localhost:#{port}", "send", "--code=ospack-test", "--text=mytext"
    end
    sleep 3

    assert_match shell_output("#{bin}/croc --relay=localhost:#{port} --overwrite --yes ospack-test").chomp, "mytext"
  end
end
