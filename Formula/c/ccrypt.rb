class Ccrypt < Formula
  ...
  test do
    touch "ospack.txt"
    system bin/"ccrypt", "-e", testpath/"ospack.txt", "-K", "secret"
    assert_predicate testpath/"ospack.txt.cpt", :exist?
    refute_predicate testpath/"ospack.txt", :exist?

    system bin/"ccrypt", "-d", testpath/"ospack.txt.cpt", "-K", "secret"
    assert_predicate testpath/"ospack.txt", :exist?
    refute_predicate testpath/"ospack.txt.cpt", :exist?
  end
end
