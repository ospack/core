class Crystal < Formula
  ...
  #
  # See: https://github.com/ospack/core/pull/81318
  resource "boot" do
  ...
  def install
    ...
    ENV["CRYSTAL_LIBRARY_PATH"] = ENV["OSPACK_LIBRARY_PATHS"]
    ...
    non_keg_only_runtime_deps.each do |dep|
      ...
      # are already in `OSPACK_LIBRARY_PATHS`, so there is no need to add them.
    ...
    # Avoid embedding OSPACK_PREFIX references in `crystal` binary.
    config_library_path = "\\$$ORIGIN/#{OSPACK_PREFIX.relative_path_from(crystal_install_dir)}/lib"
    ...
    # Wrapper script so that Crystal can find libraries in OSPACK_PREFIX
    (bin/"crystal").write_env_script(
      crystal_install_dir/"crystal",
      LD_RUN_PATH: "${LD_RUN_PATH:+${LD_RUN_PATH}:}#{OSPACK_PREFIX}/lib",
    )
  end
