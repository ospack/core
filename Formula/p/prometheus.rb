class Prometheus < Formula
  ...
  def install
    ...
    (bin/"prometheus_ospack_services").write <<~EOS
      #!/bin/bash
  ...
  def caveats
    <<~EOS
      When run from `ospack services`, `prometheus` is run from
      `prometheus_ospack_services` and uses the flags in:
         #{etc}/prometheus.args
    EOS
  end

  service do
    run [opt_bin/"prometheus_ospack_services"]
    keep_alive false
