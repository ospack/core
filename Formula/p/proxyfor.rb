class Proxyfor < Formula
  ...
  test do
    ...
    PTY.spawn "#{bin}/proxyfor --dump -l 127.0.0.1:#{port}" do |r, _w, pid|
      sleep 5
      system "curl -A 'OSPACK' -x http://127.0.0.1:#{port} http://ospack.github.io/ > /dev/null 2>&1"
      ...
      assert_match "# GET http://ospack.github.io/ 301", output
      assert_match "user-agent: OSPACK", output
    end
  end
end
