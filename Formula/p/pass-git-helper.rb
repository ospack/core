class PassGitHelper < Formula
  ...
  test do
    ...
    pipe_output("#{Formula["gnupg"].opt_bin}/gpg --generate-key --batch", <<~EOS, 0)
      ...
      %transient-key
      Key-Type: RSA
      Name-Real: Ospack Test
    EOS

    system "pass", "init", "Ospack Test"

    pipe_output("pass insert -m -f ospack/pass-git-helper-test", <<~EOS, 0)
      test_password
      test_username
    EOS
