class PostgresqlAT17 < Formula
  ...
  def install
    ...
    # using #{OSPACK_PREFIX}/lib path set during ./configure, which will cause audit failures
    ...
    system "./configure", *args, *std_configure_args(libdir: OSPACK_PREFIX/"lib/#{name}")
    system "make"
    # We use an unversioned `postgresql` subdirectory rather than `#{name}` so that the
    # post-installed symlinks can use non-conflicting `#{name}` and be retained on `ospack unlink`.
  ...
  def post_install
    ...
    # Manually link files from keg to non-conflicting versioned directories in OSPACK_PREFIX.
    %w[include lib share].each do |dir|
      dst_dir = OSPACK_PREFIX/dir/name
    ...
    bin.each_child { |f| (OSPACK_PREFIX/"bin").install_symlink f => "#{f.basename}-#{version.major}" }
  ...
  test do
    ...
    [bin/"pg_config", OSPACK_PREFIX/"bin/pg_config-#{version.major}"].each do |pg_config|
      assert_equal "#{OSPACK_PREFIX}/share/#{name}", shell_output("#{pg_config} --sharedir").chomp
      assert_equal "#{OSPACK_PREFIX}/lib/#{name}", shell_output("#{pg_config} --libdir").chomp
      assert_equal "#{OSPACK_PREFIX}/lib/#{name}", shell_output("#{pg_config} --pkglibdir").chomp
      assert_equal "#{OSPACK_PREFIX}/include/#{name}", shell_output("#{pg_config} --pkgincludedir").chomp
      assert_equal "#{OSPACK_PREFIX}/include/#{name}/server", shell_output("#{pg_config} --includedir-server").chomp
