class Poetry < Formula
  ...
  test do
    ...
    # https://github.com/ospack/core/pull/109777#issuecomment-1248353918
    ENV["PYTHON_KEYRING_BACKEND"] = "keyring.backends.null.Keyring"

    assert_match version.to_s, shell_output("#{bin}/poetry --version")
    assert_match "Created package", shell_output("#{bin}/poetry new ospack")

    cd testpath/"ospack" do
      system bin/"poetry", "config", "virtualenvs.in-project", "true"
      system bin/"poetry", "add", "requests"
      system bin/"poetry", "add", "boto3"
    end

    assert_predicate testpath/"ospack/pyproject.toml", :exist?
    assert_predicate testpath/"ospack/poetry.lock", :exist?
    assert_match "requests", (testpath/"ospack/pyproject.toml").read
    assert_match "boto3", (testpath/"ospack/pyproject.toml").read
  end
end
