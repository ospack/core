class Pypy310 < Formula
  ...
  # See https://github.com/ospack/core/pull/99892#issuecomment-1108492321
  ...
  def install
    ...
    inreplace "lib_pypy/_tkinter/tklib_build.py" do |s|
      ...
      # TODO: upstream this.
      s.gsub! "(ospack + '/include')", "('#{tcltk.opt_include}/tcl-tk')"
      s.gsub! "(ospack + '/opt/tcl-tk/lib')", "('#{tcltk.opt_lib}')"
    end
    ...
    if OS.mac?
      ...
      inreplace "lib-python/3/ctypes/macholib/dyld.py" do |f|
        f.gsub! "DEFAULT_LIBRARY_FALLBACK = [",
                "DEFAULT_LIBRARY_FALLBACK = [ '#{OSPACK_PREFIX}/lib', "
        f.gsub! "DEFAULT_FRAMEWORK_FALLBACK = [", "DEFAULT_FRAMEWORK_FALLBACK = [ '#{OSPACK_PREFIX}/Frameworks',"
      end
    end

    # Having PYTHONPATH set can cause the build to fail if another
    # Python is present, e.g. a Ospack-provided Python 2.x
    # See https://github.com/Ospack/ospack/issues/24364
    ENV["PYTHONPATH"] = nil
  ...
  def post_install
    ...
    site_packages(OSPACK_PREFIX).mkpath
    touch site_packages(OSPACK_PREFIX)/".keepme"
    rm_r(site_packages(libexec))

    # Symlink the prefix site-packages into the cellar.
    site_packages(libexec).parent.install_symlink site_packages(OSPACK_PREFIX)
    ...
    (OSPACK_PREFIX/"bin").install_symlink symlink_to_prefix
  end
  ...
  def caveats
    <<~EOS
      ...
      above, so you may want to add it to your PATH *after* #{OSPACK_PREFIX}/bin
  ...
  def scripts_folder
    OSPACK_PREFIX/"share/pypy#{abi_version}"
  end
...
+else:
     # ospack
     ospack = os.environ.get('OSPACK_PREFIX', '')
     incdirs = ['/usr/local/opt/tcl-tk/include']
@@ -26,7 +26,7 @@ elif sys.platform == 'darwin':
     if ospack:
         incdirs.append(ospack + '/include')
         libdirs.append(ospack + '/opt/tcl-tk/lib')
-else:
