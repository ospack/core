class Proxify < Formula
  ...
  url "https://github.com/projectdiscovery/proxify/archive/refs/tags/v0.0.15.tar.gz"
  ...
  test do
    # Other commands start proxify, which causes Ospack CI to time out
    assert_match version.to_s, shell_output("#{bin}/proxify -version 2>&1")
    assert_match "given config file 'ospack' does not exist", shell_output("#{bin}/proxify -config ospack 2>&1", 1)
  end
end
