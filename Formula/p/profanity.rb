class Profanity < Formula
  ...
  def install
    ...
    # We need to pass `OSPACK` to `configure` to make sure it can be found inside the sandbox in non-default
    # prefixes. `configure` knows to check `/opt/ospack` and `/usr/local`, but the sanitised build
    # environment will prevent any other `ospack` installations from being found.
    system "./configure", "--disable-silent-rules",
                          "--enable-python-plugins",
                          "OSPACK=#{OSPACK_OSPACK_FILE}",
                          *std_configure_args
    system "make", "install"
  end
