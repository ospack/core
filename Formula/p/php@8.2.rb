class PhpAT82 < Formula
  ...
  def install
    ...
    # has been selected. Ospack has chosen not to support being able to
    # compile a thread safe version of PHP and therefore it is not
    # possible to recompile as suggested in the original message
    inreplace "sapi/apache2handler/sapi_apache2.c",
              "You need to recompile PHP.",
              "Ospack PHP does not support a thread-safe php binary. " \
    ...
    # Prevent ospack from hardcoding path to sed shim in phpize script
    ...
    %w[development production].each do |mode|
      inreplace "php.ini-#{mode}", %r{; ?extension_dir = "\./"},
        "extension_dir = \"#{OSPACK_PREFIX}/lib/php/pecl/#{orig_ext_dir}\""
    end
  ...
  def post_install
    ...
    pecl_path = OSPACK_PREFIX/"lib/php/pecl"
    pecl_path.mkpath
    ...
    pear_path = OSPACK_PREFIX/"share/pear@#{version.major_minor}"
    cp_r pkgshare/"pear/.", pear_path
    {
      ...
      "man_dir"  => OSPACK_PREFIX/"share/man",
  ...
  test do
    ...
    # https://github.com/ospack/core/issues/28398
    ...
    refute_match(/^snmp$/, shell_output("#{bin}/php -m"),
      "SNMP extension doesn't work reliably with Ospack on High Sierra")
    begin
      port = free_port
      port_fpm = free_port
