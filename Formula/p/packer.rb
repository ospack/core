class Packer < Formula
  ...
  def install
    ...
    # Allow packer to find plugins in Ospack prefix
    bin.env_script_all_files libexec/"bin", PACKER_PLUGIN_PATH: "$PACKER_PLUGIN_PATH:#{OSPACK_PREFIX/"bin"}"
  ...
  def caveats
    <<~EOS
      We will not accept any new packer releases in ospack/core (with the BUSL license).
      The next release will change to a non-open-source license:
      https://www.hashicorp.com/blog/hashicorp-adopts-business-source-license
      See our documentation for acceptable licences:
        https://docs.ospack.github.io/License-Guidelines
    EOS
  end
    ...
    minimal.write <<~JSON
      {
        "builders": [{
          ...
          "ssh_username": "ubuntu",
          "ami_name": "ospack packer test  {{timestamp}}"
        }],
        "provisioners": [{
          "type": "shell",
          "inline": [
