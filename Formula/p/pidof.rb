class Pidof < Formula
  ...
  test do
    ...
    (testpath/"ospack_testing.c").write <<~C
      #include <unistd.h>
      #include <stdio.h>
    ...
    C
    system ENV.cc, "ospack_testing.c", "-o", "ospack_testing"
    (testpath/"ospack_testing").chmod 0555

    pid = fork { exec "./ospack_testing" }
    sleep 1
    begin
      assert_match(/\d+/, shell_output("#{bin}/pidof ospack_testing"))
    ensure
