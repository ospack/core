class PythonAT39 < Formula
  ...
  def post_install
    ...
    # Help distutils find ospacked stuff when building extensions
    include_dirs = [OSPACK_PREFIX/"include", Formula["openssl@3"].opt_include,
                    Formula["sqlite"].opt_include]
    library_dirs = [OSPACK_PREFIX/"lib", Formula["openssl@3"].opt_lib,
                    Formula["sqlite"].opt_lib]

    cfg = lib_cellar/"distutils/distutils.cfg"
    cfg.atomic_write <<~INI
      [install]
      prefix=#{OSPACK_PREFIX}
      [build_ext]
