class PythonAT311 < Formula
  ...
  patch do
    url "https://raw.githubusercontent.com/Ospack/formula-patches/6d2fba8de3159182025237d373a6f4f78b8bd203/python/3.11-sysconfig.diff"
  ...
  patch do
    url "https://raw.githubusercontent.com/Ospack/formula-patches/a1618a5005d0b01d63b720321806820a03432f1a/python/3.10-distutils-scheme.diff"
  ...
  def site_packages
    OSPACK_PREFIX/"lib/python#{version.major_minor}/site-packages"
  end
  ...
  def install
    ...
    if OS.linux?
      ...
      # a 32-bit system ncurses (conflicts with the ospacked 64-bit one).
      # See https://github.com/Ospack/linuxospack-core/pull/22307#issuecomment-781896552
  ...
  def post_install
    ...
    %W[wheel#{version.major_minor} pip#{version.major_minor}].each do |e|
      (OSPACK_PREFIX/"bin").install_symlink bin/e
    end
  end
  ...
  def sitecustomize
    <<~PYTHON
      ...
      for split_module in ["tk", "gdbm"]:
          split_prefix = f"#{OSPACK_PREFIX}/opt/python-{split_module}@#{version.major_minor}/libexec"
  ...
  def caveats
    <<~EOS
      Python is installed as
        #{OSPACK_PREFIX}/bin/python#{version.major_minor}
      ...
      tkinter is no longer included with this formula, but it is available separately:
        ospack install python-tk@#{version.major_minor}

      gdbm (`dbm.gnu`) is no longer included in this formula, but it is available separately:
        ospack install python-gdbm@#{version.major_minor}
      `dbm.ndbm` changed database backends in Ospack Python 3.11.
      If you need to read a database from a previous Ospack Python created via `dbm.ndbm`,
      you'll need to read your database using the older version of Ospack Python and convert to another format.
      `dbm` still defaults to `dbm.gnu` when it is installed.

      If you do not need a specific version of Python, and always want Ospack's `python3` in your PATH:
        ospack install python3

      For more information about Ospack and Python, see: https://docs.ospack.github.io/Ospack-and-Python
    EOS
  end
