class Whaleospack < Formula
  desc "Ospack, but with Docker images"
  homepage "https://github.com/whaleospack/whaleospack"
  url "https://github.com/whaleospack/whaleospack/archive/refs/tags/0.5.0.tar.gz"
  sha256 "2abea4171dbdca429b6476cffdfe7c94ce27028dc5d96e0f3a9a4fdeab77c4fb"
  license "Apache-2.0"
  head "https://github.com/whaleospack/whaleospack.git", branch: "master"
  ...
  # Run "go mod tidy": https://github.com/whaleospack/whaleospack/pull/299
  patch do
    url "https://github.com/whaleospack/whaleospack/commit/f64b9db9a5f1c91571a622a58eb93233f8c59642.patch?full_index=1"
  ...
  def install
    ldflags = %W[
      -s -w
      -X github.com/whaleospack/whaleospack/version.Version=#{version}+ospack
    ]
    system "go", "build", *std_go_args(ldflags:)
    generate_completions_from_executable(bin/"whaleospack", "completion")
  end

  test do
    assert_match "Whaleospack #{version}+ospack", shell_output("#{bin}/whaleospack version")
    assert_match "whaleospack/whalesay", shell_output("#{bin}/whaleospack search whalesay")

    output = shell_output("#{bin}/whaleospack install whaleospack/whalesay -y 2>&1", 255)
