class Wails < Formula
  ...
  test do
    ...
    output = shell_output("#{bin}/wails init -n ospacktest 2>&1")
    assert_match "# Initialising Project 'ospacktest'", output
    assert_match "Template          | Vanilla + Vite", output

    assert_path_exists testpath/"ospacktest/go.mod"
    assert_equal "ospacktest", JSON.parse((testpath/"ospacktest/wails.json").read)["name"]
