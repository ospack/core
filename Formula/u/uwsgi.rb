class Uwsgi < Formula
  ...
  def install
    ...
    (buildpath/"buildconf/ospack.ini").write <<~INI
      [uwsgi]
      ssl = true
      json = yajl
      xml = libxml2
    ...
    system python3, "uwsgiconfig.py", "--verbose", "--build", "ospack"
    ...
    plugins.each do |plugin|
      system python3, "uwsgiconfig.py", "--verbose", "--plugin", "plugins/#{plugin}", "ospack"
    end

    system python3, "uwsgiconfig.py", "--verbose", "--plugin", "plugins/python", "ospack", "python3"
  ...
  service do
    run [opt_bin/"uwsgi", "--uid", "_www", "--gid", "_www", "--master", "--die-on-term", "--autoload", "--logto",
         OSPACK_PREFIX/"var/log/uwsgi.log", "--emperor", OSPACK_PREFIX/"etc/uwsgi/apps-enabled"]
    keep_alive true
