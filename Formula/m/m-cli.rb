class MCli < Formula
  ...
  def install
    ...
    inreplace prefix/"m" do |s|
      ...
      # Disable options "update" && "uninstall", they must be handled by ospack
      s.gsub!(/update_mcli &&.*/, "printf \"Try: ospack update && ospack upgrade m-cli \\n\" && exit 0")
      s.gsub!(/uninstall_mcli &&.*/, "printf \"Try: ospack uninstall m-cli \\n\" && exit 0")
    end
