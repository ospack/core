class Mkcert < Formula
  ...
  test do
    ENV["CAROOT"] = testpath
    system bin/"mkcert", "ospack.test"
    assert_predicate testpath/"ospack.test.pem", :exist?
    assert_predicate testpath/"ospack.test-key.pem", :exist?
    output = (testpath/"ospack.test.pem").read
    assert_match "-----BEGIN CERTIFICATE-----", output
    output = (testpath/"ospack.test-key.pem").read
