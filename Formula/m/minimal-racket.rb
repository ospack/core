class MinimalRacket < Formula
  ...
  def post_install
    ...
    return unless racket_config.read.include?(OSPACK_CELLAR)

    ohai "Fixing up Cellar references in #{racket_config}..."
    inreplace racket_config, %r{#{Regexp.escape(OSPACK_CELLAR)}/minimal-racket/[^/]}o, opt_prefix
  end
  ...
  def caveats
    <<~EOS
      ...
      The full Racket distribution is available as a cask:
        ospack install --cask racket
    EOS
  end

  test do
    output = shell_output("#{bin}/racket -e '(displayln \"Hello Ospack\")'")
    assert_match "Hello Ospack", output
