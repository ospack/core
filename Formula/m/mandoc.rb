class Mandoc < Formula
  ...
  def install
    localconfig = [
      ...
      # sudoer privileges, or will error. So just ospack's manpages for now?
      "MANPATH_DEFAULT=#{OSPACK_PREFIX}/share/man",

      "HAVE_MANPATH=0",   # Our `manpath` is a symlink to system `man`.
      "STATIC=",          # No static linking on Darwin.

      "OSPACKDIR=#{OSPACK_CELLAR}", # ? See configure.local.example, NEWS.
      "BUILD_CGI=1",
    ]
