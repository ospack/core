class MetaPackageManager < Formula
  ...
  test do
    ...
    # Check mpm is detecting ospack and report it as a manager in a table row.
    assert_match(
      /\e\[32mospack\e\[0m,Ospack Formulae,\e\[32m✓\e\[0m,\e\[32m✓\e\[0m \S+,\e\[32m✓\e\[0m,\e\[32m✓\e\[0m \S+/,
      shell_output("#{bin}/mpm --output-format csv --all-managers managers"),
    )
    # Check mpm is reporting itself as installed via ospack in a table row.
    assert_match "meta-package-manager,,ospack,#{version}", shell_output("#{bin}/mpm --output-format csv installed")
  end
end
