class MecabKo < Formula
  ...
  def install
    ...
    # Put dic files in OSPACK_PREFIX/lib instead of lib
    inreplace "#{bin}/mecab-config", "${exec_prefix}/lib/mecab/dic", "#{OSPACK_PREFIX}/lib/mecab/dic"
    inreplace "#{etc}/mecabrc", "#{lib}/mecab/dic", "#{OSPACK_PREFIX}/lib/mecab/dic"
  end

  def post_install
    (OSPACK_PREFIX/"lib/mecab/dic").mkpath
  end

  test do
    assert_equal "#{OSPACK_PREFIX}/lib/mecab/dic", shell_output("#{bin}/mecab-config --dicdir").chomp
  end
end
