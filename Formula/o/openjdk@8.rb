  ...
  def install
    ...
    if OS.mac?
      ...
      inreplace "hotspot/make/bsd/makefiles/gcc.make" do |s|
        s.gsub! "$(subst .,,$(MACOSX_VERSION_MIN))", ENV["OSPACK_MACOS_VERSION_NUMERIC"]
        s.gsub! "MACOSX_VERSION_MIN=10.7.0", "MACOSX_VERSION_MIN=#{MacOS.version}"
      end
    else
      # Fix linker errors on ospacked GCC.
    ...
    if OS.mac?
      ...
      args += %W[
        ...
        --x-includes=#{OSPACK_PREFIX}/include
        --x-libraries=#{OSPACK_PREFIX}/lib
