class Oras < Formula
  ...
  def install
    ldflags = %W[
      ...
      -X oras.land/oras/internal/version.BuildMetadata=Ospack
    ]
  ...
  test do
    assert_match "#{version}+Ospack", shell_output("#{bin}/oras version")
    ...
    output = shell_output("#{bin}/oras push localhost:#{port}/test-artifact:v1 " \
                          "--config test.json:application/vnd.ospack.test.config.v1+json " \
                          "./test.json 2>&1", 1)
