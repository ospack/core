class Opentsdb < Formula
  ...
  def install
    ...
    inreplace pkgshare/"etc/opentsdb/opentsdb.conf", "/usr/share", "#{OSPACK_PREFIX}/share"
  ...
  test do
    ...
    begin
      ...
      pipe_output("nc localhost 4242 2>&1", "put ospack.install.test 1356998400 42.5 host=webserver01 cpu=0\n")

      system bin/"tsdb", "query", "1356998000", "1356999000", "sum",
             "ospack.install.test", "host=webserver01", "cpu=0"
    ensure
      system "#{Formula["hbase"].opt_bin}/stop-hbase.sh"
    end
  end
