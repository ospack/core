class Koka < Formula
  ...
  def install
    inreplace "src/Compile/Options.hs" do |s|
      s.gsub! '["/usr/local/lib"', "[\"#{OSPACK_PREFIX}/lib\""
  ...
  test do
    (testpath/"helloospack.kk").write('pub fun main() println("Hello Ospack")')
    assert_match "Hello Ospack", shell_output("#{bin}/koka -e helloospack.kk")
