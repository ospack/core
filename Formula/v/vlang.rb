class Vlang < Formula
  ...
  resource "vc" do
    ...
    on_big_sur :or_older do
      patch do
        url "https://raw.githubusercontent.com/Ospack/formula-patches/4a51a527e534534c3ddc6801c45d3a3a2c8fbd5a/vlang/vc.patch"
  ...
  def install
    ...
    %w[up self].each do |cmd|
      ...
      (buildpath/"cmd/tools/v#{cmd}.v").write <<~EOS
        println('ERROR: `v #{cmd}` is disabled. Use `ospack upgrade #{name}` to update V.')
      EOS
    end
