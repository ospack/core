class VorbisTools < Formula
  ...
  url "https://downloads.xiph.org/releases/vorbis/vorbis-tools-1.4.2.tar.gz", using: :ospack_curl
  ...
  def install
    if OS.mac?
      # Prevent linkage with Ospack Curl on macOS because of `using: :ospack_curl` above.
      ENV.remove "OSPACK_DEPENDENCIES", "curl"
      ENV.remove "OSPACK_INCLUDE_PATHS", Formula["curl"].opt_include
      ENV.remove "OSPACK_LIBRARY_PATHS", Formula["curl"].opt_lib
