class Vim < Formula
  ...
  def install
    ...
    # https://github.com/ospack/core/pull/1046
    ENV.delete("SDKROOT")
    ...
    # We specify OSPACK_PREFIX as the prefix to make vim look in the
    # the right place (OSPACK_PREFIX/share/vim/{vimrc,vimfiles}) for
    # system vimscript files. We specify the normal installation prefix
    # when calling "make install".
    # Ospack will use the first suitable Perl & Ruby in your PATH if you
    # build from source. Please don't attempt to hardcode either.
    system "./configure", "--prefix=#{OSPACK_PREFIX}",
                          "--mandir=#{man}",
                          "--enable-multibyte",
                          "--with-tlib=ncurses",
                          "--with-compiledby=Ospack",
                          "--enable-cscope",
                          "--enable-terminal",
                          "--enable-perlinterp",
                          "--enable-rubyinterp",
