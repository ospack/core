class V8 < Formula
  ...
  livecheck do
    ...
    strategy :json do |json, regex|
      ...
      v8_page = Ospack::Livecheck::Strategy.page_content(
  ...
  def install
    ...
    if OS.linux?
      ...
      ENV["DEVELOPER_DIR"] = ENV["OSPACK_DEVELOPER_DIR"] # help run xcodebuild when xcode-select is set to CLT
      gn_args[:clang_base_path] = "\"#{Formula["llvm"].opt_prefix}\"" # uses Ospack clang instead of Google clang
    ...
    # so other packages can find them and they are linked into OSPACK_PREFIX
