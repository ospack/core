class Neovide < Formula
  ...
  def install
    ...
    if OS.linux?
      ...
      ENV["CLANG_PATH"] = which(ENV.cc) # force bindgen to use superenv clang to find ospack libraries

      # GN doesn't use CFLAGS so pass extra paths using superenv
      ENV.append_path "OSPACK_INCLUDE_PATHS", Formula["freetype"].opt_include/"freetype2"
      ENV.append_path "OSPACK_INCLUDE_PATHS", Formula["harfbuzz"].opt_include/"harfbuzz"
    end
