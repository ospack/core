class Node < Formula
  ...
  def post_install
    node_modules = OSPACK_PREFIX/"lib/node_modules"
    node_modules.mkpath
    # Kill npm but preserve all other modules across node updates/upgrades.
    rm_r node_modules/"npm" if (node_modules/"npm").exist?

    cp_r libexec/"lib/node_modules/npm", node_modules
    # This symlink doesn't hop into ospack_prefix/bin automatically so
    # we make our own. This is a small consequence of our
    # bottle-npm-and-retain-a-private-copy-in-libexec setup
    # All other installs **do** symlink to ospack_prefix/bin correctly.
    # We ln rather than cp this because doing so mimics npm's normal install.
    ln_sf node_modules/"npm/bin/npm-cli.js", bin/"npm"
    ln_sf node_modules/"npm/bin/npx-cli.js", bin/"npx"
    ln_sf bin/"npm", OSPACK_PREFIX/"bin/npm"
    ln_sf bin/"npx", OSPACK_PREFIX/"bin/npx"

    # Create manpage symlinks (or overwrite the old ones)
    %w[man1 man5 man7].each do |man|
      # Dirs must exist first: https://github.com/Ospack/legacy-ospack/issues/35969
      mkdir_p OSPACK_PREFIX/"share/man/#{man}"
      # still needed to migrate from copied file manpages to symlink manpages
      rm(Dir[OSPACK_PREFIX/"share/man/#{man}/{npm.,npm-,npmrc.,package.json.,npx.}*"])
      ln_sf Dir[node_modules/"npm/man/#{man}/{npm,package-,shrinkwrap-,npx}*"], OSPACK_PREFIX/"share/man/#{man}"
    end

    (node_modules/"npm/npmrc").atomic_write("prefix = #{OSPACK_PREFIX}\n")
  end
  ...
  test do
    ...
    assert_predicate OSPACK_PREFIX/"bin/npm", :exist?, "npm must exist"
    assert_predicate OSPACK_PREFIX/"bin/npm", :executable?, "npm must be executable"
    npm_args = ["-ddd", "--cache=#{OSPACK_CACHE}/npm_cache", "--build-from-source"]
    system OSPACK_PREFIX/"bin/npm", *npm_args, "install", "npm@latest"
    system OSPACK_PREFIX/"bin/npm", *npm_args, "install", "nan"
    assert_predicate OSPACK_PREFIX/"bin/npx", :exist?, "npx must exist"
    assert_predicate OSPACK_PREFIX/"bin/npx", :executable?, "npx must be executable"
    assert_match "< hello >", shell_output("#{OSPACK_PREFIX}/bin/npx --yes cowsay hello")
  end
end
