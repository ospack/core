class Ncdu < Formula
  ...
  def install
    ...
    # https://github.com/ospack/core/issues/92282
    ...
    # Workaround for https://github.com/ospack/core/pull/141453#discussion_r1320821081
    # Remove this workaround when the same is removed in `zig.rb`.
    if OS.linux?
      ENV["NIX_LDFLAGS"] = ENV["OSPACK_RPATH_PATHS"].split(":")
                                                      .map { |p| "-rpath #{p}" }
                                                      .join(" ")
    end
