...
class Nethack < Formula
  ...
  def install
    ...
    # see https://github.com/Ospack/ospack/issues/14763.
    ENV.O0
      ...
      inreplace "hints/#{hintfile}" do |s|
        ...
        if build.stable?
          s.gsub! "#WANT_WIN_CURSES=1",
                  "WANT_WIN_CURSES=1\nCFLAGS+=-DVAR_PLAYGROUND='\"#{OSPACK_PREFIX}/share/nethack\"'"
        end
      end
  ...
  def post_install
    ...
    savedir = OSPACK_PREFIX/"share/nethack"
    mkdir_p savedir
    cd savedir do
      ...
      mkdir_p "save"
      touch "save/.keepme" # preserve on `ospack cleanup`
    end
  ...
  test do
    ...
    assert_match (OSPACK_PREFIX/"share/nethack").to_s,
