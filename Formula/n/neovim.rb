class Neovim < Formula
  ...
  stable do
    ...
    #       https://github.com/orgs/Ospack/discussions/3611
  ...
  def install
    ...
    # Point system locations inside `OSPACK_PREFIX`.
    inreplace "src/nvim/os/stdpaths.c" do |s|
      s.gsub! "/etc/xdg/", "#{etc}/xdg/:\\0"

      if OSPACK_PREFIX.to_s != OSPACK_DEFAULT_PREFIX
        s.gsub! "/usr/local/share/:/usr/share/", "#{OSPACK_PREFIX}/share/:\\0"
      end
    end

    # Replace `-dirty` suffix in `--version` output with `-Ospack`.
    inreplace "cmake/GenerateVersion.cmake", "--dirty", "--dirty=-Ospack"
