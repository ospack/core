class NodeExporter < Formula
  ...
  def install
    ldflags = %W[
      ...
      -X github.com/prometheus/common/version.BuildUser=Ospack
    ]
    system "go", "build", *std_go_args(ldflags:)

    touch etc/"node_exporter.args"

    (bin/"node_exporter_ospack_services").write <<~BASH
      #!/bin/bash
  ...
  def caveats
    <<~EOS
      When run from `ospack services`, `node_exporter` is run from
      `node_exporter_ospack_services` and uses the flags in:
        #{etc}/node_exporter.args
    EOS
  end

  service do
    run [opt_bin/"node_exporter_ospack_services"]
    keep_alive false
