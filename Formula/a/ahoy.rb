class Ahoy < Formula
  ...
  def install
    cd "v2" do
      system "go", "build", *std_go_args(ldflags: "-s -w -X main.version=#{version}-ospack")
    end
  ...
  test do
    (testpath/".ahoy.yml").write <<~YAML
      ...
      commands:
        hello:
          cmd: echo "Hello Ospack!"
    YAML
    assert_equal "Hello Ospack!\n", `#{bin}/ahoy hello`

    assert_equal "#{version}-ospack", shell_output("#{bin}/ahoy --version").strip
  end
end
