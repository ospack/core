class Ant < Formula
  ...
  def install
    ...
    (bin/"ant").write <<~SHELL
      #!/bin/bash
      JAVA_HOME="${JAVA_HOME:-#{Formula["openjdk"].opt_prefix}}" exec "#{libexec}/bin/ant" -lib #{OSPACK_PREFIX}/share/ant "$@"
    SHELL
    ...
    resource("bcel").stage do
      ...
      # Ensure bcel jar is readable by world, see https://github.com/ospack/core/issues/203905
  ...
  test do
    (testpath/"build.xml").write <<~XML
      <project name="OspackTest" basedir=".">
    ...
    (testpath/"src/main/java/org/ospack/AntTest.java").write <<~JAVA
      package org.ospack;
      public class AntTest {
        public static void main(String[] args) {
          System.out.println("Testing Ant with Ospack!");
        }
      }
    JAVA
