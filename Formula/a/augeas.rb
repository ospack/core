class Augeas < Formula
  ...
  def caveats
    <<~EOS
      Lenses have been installed to:
        #{OSPACK_PREFIX}/share/augeas/lenses/dist
    EOS
  end
  ...
  test do
    ...
    (testpath/"etc/hosts").write <<~EOS
      192.168.0.1 ospack.github.io test
    EOS

    expected_augtool_output = <<~EOS
      /files/etc/hosts/1
      /files/etc/hosts/1/ipaddr = "192.168.0.1"
      /files/etc/hosts/1/canonical = "ospack.github.io"
    ...
    expected_augprint_output = <<~EOS
      ...
      set /files/etc/hosts/seq::*[ipaddr='192.168.0.1']/canonical 'ospack.github.io'
