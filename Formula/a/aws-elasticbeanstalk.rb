class AwsElasticbeanstalk < Formula
  ...
  test do
    output = shell_output("#{bin}/eb init --region=us-east-1 --profile=ospack-test", 4)
    assert_match("ERROR: InvalidProfileError - The config profile (ospack-test) could not be found", output)
  end
end
