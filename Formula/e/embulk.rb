class Embulk < Formula
  ...
  # The same OpenJDK version must be used on all platforms due to a ospack bug relocating `:all` bottles.
  # See: https://github.com/Ospack/ospack/issues/18695
  ...
  test do
    # In order to install a different plugin, we need JRuby, but ospack `jruby`
    ...
    (testpath/"config.yml").write <<~YAML
      in:
        type: http
        url: https://formulae.ospack.github.io/api/analytics/ospack-command-run/30d.json
        parser:
          type: json
          root: /items
