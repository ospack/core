class R < Formula
  ...
  def install
    ...
    args = [
      ...
      # and helps CRAN packages find gfortran when Ospack may not be
    ...
    if OS.mac?
      ...
      # search those directories. Remove -LOSPACK_PREFIX/lib from LDFLAGS.
      ENV.remove "LDFLAGS", "-L#{OSPACK_PREFIX}/lib"
    ...
    # make Ospack packages discoverable for R CMD INSTALL
    inreplace r_home/"etc/Makeconf" do |s|
      s.gsub!(/^CPPFLAGS =.*/, "\\0 -I#{OSPACK_PREFIX}/include")
      s.gsub!(/^LDFLAGS =.*/, "\\0 -L#{OSPACK_PREFIX}/lib")
  ...
  def post_install
    ...
    site_library = OSPACK_PREFIX/"lib/R"/short_version/"site-library"
    site_library.mkpath
