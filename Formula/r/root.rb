class Root < Formula
  ...
  def install
    ...
    # Related: https://github.com/ospack/core/issues/135714
    ...
    inreplace "cmake/modules/SearchInstalledSoftware.cmake" do |s|
      ...
      # Patch out check that skips using ospacked glew.
    ...
    # Ospack now sets CMAKE_INSTALL_LIBDIR to /lib, which is incorrect
    ...
    (prefix/Language::Python.site_packages(python3)/"ospack-root.pth").write pth_contents
  end
  ...
  def caveats
    <<~TEXT
      ...
      For bash users:
        . #{OSPACK_PREFIX}/bin/thisroot.sh
      For zsh users:
        pushd #{OSPACK_PREFIX} >/dev/null; . bin/thisroot.sh; popd >/dev/null
      For csh/tcsh users:
        source #{OSPACK_PREFIX}/bin/thisroot.csh
      For fish users:
        . #{OSPACK_PREFIX}/bin/thisroot.fish
    TEXT
  end
