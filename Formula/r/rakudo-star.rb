class RakudoStar < Formula
  ...
  def install
    ...
    # Help Readline module find ospack `readline` on Linux
    ...
    # openssl module's ospack --prefix openssl probe fails so
    # set value here
  ...
  test do
    ...
    (testpath/"openssl.raku").write <<~PERL
      ...
      my $ciphertext = encrypt("ospack".encode, :aes256, :iv(("0" x 16).encode), :key(('x' x 32).encode));
      print decrypt($ciphertext, :aes256, :iv(("0" x 16).encode), :key(('x' x 32).encode)).decode;
    PERL
    assert_equal "ospack", shell_output("#{bin}/raku openssl.raku")
    ...
    (testpath/"readline.raku").write <<~PERL
      ...
      print "[$response]";
    PERL
    assert_equal "test> ospack\n[ospack]", pipe_output("#{bin}/raku readline.raku", "ospack\n", 0)
