class Rabbitmq < Formula
  ...
  def install
    ...
    inreplace sbin/"rabbitmq-defaults" do |s|
      s.gsub! "SYS_PREFIX=${RABBITMQ_HOME}", "SYS_PREFIX=#{OSPACK_PREFIX}"
  ...
  def caveats
    <<~EOS
      ...
      Ospack-specific docs: https://rabbitmq.com/install-ospack.html
    EOS
  end
  ...
  def rabbitmq_env
    <<~EOS
      ...
      PLUGINS_DIR="#{opt_prefix}/plugins:#{OSPACK_PREFIX}/share/rabbitmq/plugins"
    EOS
  end
  ...
  service do
    ...
    # need erl in PATH
    environment_variables PATH:          "#{OSPACK_PREFIX}/sbin:/usr/sbin:/usr/bin:/bin:#{OSPACK_PREFIX}/bin",
