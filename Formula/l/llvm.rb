class Llvm < Formula
  ...
  def install
    ...
    # accidentally importing it with a non-Ospack Python or a Ospack Python
    ...
    if tap.present?
      ...
      args << "-DCLANG_VENDOR_UTI=sh.ospack.clang" if tap.official?
    end
    ...
    if pgo_build
      ...
      # https://github.com/ospack/core/issues/77975
      ...
      if OS.linux?
        # Make sure ospacked glibc will be used if it is installed.
        linux_library_paths = [
  ...
  def caveats
    s = <<~EOS
      ...
      We plan to build LLVM 20 with `LLVM_ENABLE_EH=OFF`. Please see:
        https://github.com/orgs/Ospack/discussions/5654
    EOS
  ...
  test do
    ...
    with_env(CPATH: nil) do
      ...
      if OS.mac? && MacOS::CLT.installed?
        ...
        if Hardware::CPU.arm?
          old_macos_version = OSPACK_MACOS_OLDEST_SUPPORTED.to_i - 1
      ...
      # related to https://github.com/Ospack/legacy-ospack/issues/47149
