class Lesspipe < Formula
  ...
  def caveats
    <<~EOS
      add the following to your shell profile e.g. ~/.profile or ~/.zshrc:
        export LESSOPEN="|#{OSPACK_PREFIX}/bin/lesspipe.sh %s"
    EOS
  end
  ...
  test do
    touch "file1.txt"
    touch "file2.txt"
    system "tar", "-cvzf", "ospack.tar.gz", "file1.txt", "file2.txt"

    assert_predicate testpath/"ospack.tar.gz", :exist?
    assert_match "file2.txt", pipe_output(bin/"archive_color", shell_output("tar -tvzf ospack.tar.gz"))
  end
end
