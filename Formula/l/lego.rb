class Lego < Formula
  ...
  test do
    output = shell_output("#{bin}/lego -a --email test@ospack.github.io --dns digitalocean -d ospack.test run 2>&1", 1)
    assert_match "some credentials information are missing: DO_AUTH_TOKEN", output

    output = shell_output(
      "DO_AUTH_TOKEN=xx #{bin}/lego -a --email test@ospack.github.io --dns digitalocean -d ospack.test run 2>&1", 1
    )
