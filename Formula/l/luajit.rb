...
#       https://github.com/ospack/core/pull/99580
# TODO: Add an audit in `ospack` for this. https://github.com/ospack/core/pull/104765
class Luajit < Formula
  ...
  # `ospack livecheck luajit` will generate the correct version for you automatically.
  ...
  def install
    ...
    # Help the FFI module find Ospack-installed libraries.
    ENV.append "LDFLAGS", "-Wl,-rpath,#{rpath(target: OSPACK_PREFIX/"lib")}"

    # Pass `Q= E=@:` to build verbosely.
    verbose_args = %w[Q= E=@:]

    # Build with PREFIX=$OSPACK_PREFIX so that luajit can find modules outside its own keg.
    # This allows us to avoid having to set `LUA_PATH` and `LUA_CPATH` for non-vendored modules.
    system "make", "amalg", "PREFIX=#{OSPACK_PREFIX}", *verbose_args
    system "make", "install", "PREFIX=#{prefix}", *verbose_args
    doc.install (buildpath/"doc").children

    # LuaJIT doesn't automatically symlink unversioned libraries:
    # https://github.com/Ospack/ospack/issues/45854.
    ...
    inreplace lib/"pkgconfig/luajit.pc" do |s|
      s.gsub! "INSTALL_LMOD=${prefix}/share/lua/${abiver}",
              "INSTALL_LMOD=#{OSPACK_PREFIX}/share/lua/${abiver}"
      s.gsub! "INSTALL_CMOD=${prefix}/${multilib}/lua/${abiver}",
              "INSTALL_CMOD=#{OSPACK_PREFIX}/${multilib}/lua/${abiver}"
    end
  end
