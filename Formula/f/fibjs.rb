class Fibjs < Formula
  ...
  # LLVM is added as a test dependency to work around limitation in Ospack's
  # test compiler selection when using fails_with. Can remove :test when fixed.
  # Issue ref: https://github.com/Ospack/ospack/issues/11795
  ...
  def install
    ...
    ENV.append "CXXFLAGS", "-I#{OSPACK_PREFIX}/include" if OS.linux?

    # the build script breaks when CI is set by Ospack
    with_env(CI: nil) do
      system "./build", "clean"
