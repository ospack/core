class Fastfec < Formula
  ...
  on_macos do
    ...
    # Use Ospack PCRE on Linux when upstream resolves
  ...
  def install
    resource("zig").stage do
      ENV["NIX_LDFLAGS"] = ENV["OSPACK_RPATH_PATHS"].split(":").map { |p| "-rpath #{p}" }.join(" ") if OS.linux?
