class Step < Formula
  ...
  test do
    ...
    system bin/"step", "ca", "init", "--address", "127.0.0.1:8081",
        ...
        "ospack-smallstep-test", "--provisioner", "ospack"
    ...
    begin
      ...
      shell_output("#{bin}/step ca token --password-file #{testpath}/password.txt " \
                   "ospack-smallstep-leaf > token.txt")
      token = File.read(testpath/"token.txt")
      system bin/"step", "ca", "certificate", "--token", token,
          "ospack-smallstep-leaf", "ospack.crt", "ospack.key"

      assert_predicate testpath/"ospack.crt", :exist?
      assert_predicate testpath/"ospack.key", :exist?
      ospack_crt = File.read(testpath/"ospack.crt")
      assert_match(/^-----BEGIN CERTIFICATE-----.*/, ospack_crt)
      assert_match(/.*-----END CERTIFICATE-----$/, ospack_crt)
      ospack_key = File.read(testpath/"ospack.key")
      assert_match(/^-----BEGIN EC PRIVATE KEY-----.*/, ospack_key)
      assert_match(/.*-----END EC PRIVATE KEY-----$/, ospack_key)
      shell_output("#{bin}/step certificate inspect --format json ospack.crt > ospack_crt.json")
      ospack_crt_json = JSON.parse(File.read(testpath/"ospack_crt.json"))
      assert_equal "CN=ospack-smallstep-leaf", ospack_crt_json["subject_dn"]
      assert_equal "O=ospack-smallstep-test, CN=ospack-smallstep-test Intermediate CA", ospack_crt_json["issuer_dn"]
    ensure
      Process.kill(9, pid)
      Process.wait(pid)
    end
