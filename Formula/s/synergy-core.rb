class SynergyCore < Formula
  ...
  # represented with the ospack `license` statement.
  #
  ...
  def install
    if OS.mac?
      ...
      inreplace "src/gui/CMakeLists.txt",
                ...
                '"MESSAGE (\\"Skipping macdeployqt in Ospack\\")"'
    elsif OS.linux?
  ...
  def caveats
    ...
    s = <<~EOS
      Read the synergy-core license here:
        #{opt_prefix}/LICENSE
    EOS
    # The binaries built by ospack are not signed by a trusted certificate, so the
