class Swift < Formula
  ...
  resource "llbuild" do
    ...
    # Workaround Ospack sqlite3 not being found.
    # Needs paired inreplace for @@OSPACK_PREFIX@@.
    # https://github.com/swiftlang/swift-llbuild/issues/901
    patch do
      url "https://raw.githubusercontent.com/Ospack/formula-patches/0080c7317c51d16b17671640c5db665516402d2f/swift/llbuild-sqlite3.patch"
  ...
  # Ospack-specific patch to make the default resource directory use opt rather than Cellar.
  ...
  patch do
    url "https://raw.githubusercontent.com/Ospack/formula-patches/5e4d9bb4d04c7c9004e95fecba362a843dc00bdd/swift/ospack-resource-dir.diff"
  ...
  def install
    ...
    # This conflicts with Ospack's sandbox.
    ...
    if OS.linux?
      ...
      ], '"$ORIGIN"', "\"$ORIGIN:#{ENV["OSPACK_RPATH_PATHS"]}\""
    end
    ...
    mkdir build do
      ...
      if OS.mac?
        ...
        ENV.remove "OSPACK_LIBRARY_PATHS", Formula["sqlite"].opt_lib
