class Subversion < Formula
  ...
  # build against Ospack versions of
  ...
  def install
    ...
    if OS.linux?
      ...
      # Fix linkage when build-from-source as ospack disables superenv when
      # `scons` is a dependency. Can remove if serf is moved to a separate
      # formula or when serf's cmake support is stable.
      ENV.append "LDFLAGS", "-Wl,-rpath=#{OSPACK_PREFIX}/lib" unless build.bottle?
    end
    ...
    if OS.mac?
      inreplace "Makefile" do |s|
        s.change_make_var! "SWIG_PL_INCLUDES",
          ...
          "-DPERL_DARWIN -fno-strict-aliasing -I#{OSPACK_PREFIX}/include -I#{perl_core}"
      end
    end
    ...
    # purged by Ospack's post-install cleaner because that doesn't check
