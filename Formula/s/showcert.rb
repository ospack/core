class Showcert < Formula
  ...
  test do
    ...
    assert_match "O=Let's Encrypt", shell_output("#{bin}/showcert ospack.github.io")

    assert_match version.to_s, shell_output("#{bin}/gencert -h")

    system bin/"gencert", "--ca", "Ospack"
    assert_path_exists testpath/"Ospack.key"
    assert_path_exists testpath/"Ospack.pem"
  end
end
