class Stuffbin < Formula
  ...
  test do
    mkdir "ospacktest" do
      system "go", "mod", "init", "ospacktest"
      system "go", "get", "github.com/knadh/stuffbin"

      (testpath/"ospacktest/foo.txt").write "ospackfoo"
      (testpath/"ospacktest/main.go").write <<~GO
        package main
      ...
      system "go", "build", "."
      output = shell_output("#{bin}/stuffbin -a stuff -in ospacktest -out ospacktest2 foo.txt")
      assert_match "stuffing complete.", output
      assert_match "foo.txt = ospackfoo", shell_output("#{testpath}/ospacktest/ospacktest2 2>&1")

      output = shell_output("#{bin}/stuffbin -a id -in ospacktest2")
      assert_match "ospacktest2: stuffbin", output
