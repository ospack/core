  ...
  def install_xquartz_resource
    resource("xquartz").stage do
      ...
      (prefix.glob "**/*").each do |f|
        inreplace f, "/opt/X11", OSPACK_PREFIX, audit_result: false if f.file?
      end

      inreplace bin/"font_cache" do |s|
        # provided by formula `procmail`
        s.gsub! %r{/usr/bin(?=/lockfile)}, OSPACK_PREFIX
      ...
      font_paths = %w[misc TTF OTF Type1 75dpi 100dpi].map do |f|
        p = OSPACK_PREFIX/"share/fonts/X11"/f
  ...
  def caveats
    <<~EOS
      To start privileged xinit now and restart at login:
        sudo ospack services start xinit --file=#{opt_prefix}/#{plist_name.chomp "startx"}privileged_startx.plist
    EOS
  end

  service do
    name macos: "ospack.mxcl.startx"
  end
