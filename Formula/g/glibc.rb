...
class OspackedGlibcNotOlderRequirement < Requirement
  fatal true
  ...
  def message
    <<~EOS
      Your system's glibc version is #{OS::Linux::Glibc.system_version}, and Ospack's glibc version is #{Glibc.version}.
...
class Glibc < Formula
  ...
  livecheck do
    skip "glibc is pinned to the version present in Ospack CI"
  end
  ...
  depends_on OspackedGlibcNotOlderRequirement
  depends_on :linux
  ...
  on_intel do
    resource "bootstrap-binutils" do
      url "https://github.com/Ospack/glibc-bootstrap/releases/download/1.0.0/bootstrap-binutils-2.38.tar.gz"
      sha256 "a2971fd77743a1d82242736c646bfa201137a4df28d829b1aa7f556fc57215e2"
    end

    resource "bootstrap-bison" do
      url "https://github.com/Ospack/glibc-bootstrap/releases/download/1.0.0/bootstrap-bison-3.8.2.tar.gz"
      sha256 "f914c0dee9fc8a200f6607d52a2d25c253b665d02aaac360711ebd5fbd9cb346"
    end

    resource "bootstrap-gawk" do
      url "https://github.com/Ospack/glibc-bootstrap/releases/download/1.0.0/bootstrap-gawk-5.1.1.tar.gz"
      sha256 "ec3f0115b156b418a189f9868aaa0655f18c40f5c40f437e407ac60b7c749e0a"
    end

    resource "bootstrap-gcc" do
      url "https://github.com/Ospack/glibc-bootstrap/releases/download/1.0.0/bootstrap-gcc-9.5.0.tar.gz"
      sha256 "d549cf096864de5da77b4f068fab3741636206f3b7ace593b46a226d726f4538"
    end

    resource "bootstrap-make" do
      url "https://github.com/Ospack/glibc-bootstrap/releases/download/1.0.0/bootstrap-make-4.3.tar.gz"
      sha256 "aa684eff83e5a986391475547c29b3ade04a307aa5730866aa5d2caa905e7166"
    end

    resource "bootstrap-python3" do
      url "https://github.com/Ospack/glibc-bootstrap/releases/download/1.0.0/bootstrap-python3-3.9.13.tar.gz"
      sha256 "93d258ab9240d247a66322926deb6728e2aa7877711196fde02d716c20ada490"
    end

    resource "bootstrap-sed" do
      url "https://github.com/Ospack/glibc-bootstrap/releases/download/1.0.0/bootstrap-sed-4.8.tar.gz"
  ...
  def install
    ...
    if Hardware::CPU.intel?
      # Set up bootstrap resources in /tmp/ospack.
      bootstrap_dir = Pathname.new("/tmp/ospack")
      ...
      ENV.append "CFLAGS", "-march=core2 -O2" if ENV["OSPACK_GITHUB_ACTIONS"]
    end

    # Setting RPATH breaks glibc.
    %w[
      LDFLAGS LD_LIBRARY_PATH LD_RUN_PATH LIBRARY_PATH
      OSPACK_DYNAMIC_LINKER OSPACK_LIBRARY_PATHS OSPACK_RPATH_PATHS
    ].each { |x| ENV.delete x }

    # Use ospacked ld.so.preload rather than the hotst's /etc/ld.so.preload
    ...
    # Add ld.so.conf (will be written to OSPACK_PREFIX/etc/ld.so.conf)
    atomic_write_with_mode etc/"ld.so.conf", <<~EOS
      # This file is generated by Ospack. Do not modify.
      #{opt_lib}  # ensure Ospack Glibc always comes first
    ...
    atomic_write_with_mode ld_so_conf_d/"README", <<~EOS
      This is the Ospack's ld configuration directory
    ...
    # Add Ospack lib to ld search paths
    atomic_write_with_mode ld_so_conf_d/"90-ospack.conf", "#{OSPACK_PREFIX}/lib"

    # Add system ld search paths (disabled by default)
    atomic_write_with_mode system_ld_so_conf, <<~EOS
      # The system ld search paths
      #
      # If you want Ospack's ld.so to search for libraries in the system paths,
      # remove the "#{system_ld_so_conf.extname}" suffix of this file.
      # Mixing the Ospack and system library search paths is very risky and can
  ...
  def post_install
    ...
    ospack_localtime = etc/"localtime"
    etc.install_symlink sys_localtime if sys_localtime.exist? && !ospack_localtime.exist?

    # Set zoneinfo correctly using the system installed zoneinfo
    sys_zoneinfo = Pathname("/usr/share/zoneinfo")
    ospack_zoneinfo = share/"zoneinfo"
    share.install_symlink sys_zoneinfo if sys_zoneinfo.exist? && !ospack_zoneinfo.exist?
  end

  def caveats
    <<~EOS
      The Ospack's Glibc has been installed with the following executables:
        #{opt_bin}/ldd
        #{opt_bin}/ld.so
        #{opt_sbin}/ldconfig

      By default, Ospack's linker will not search for the system's libraries. If you
      want Ospack to do so, run:

        cp "#{system_ld_so_conf}" "#{ld_so_conf_d}/#{system_ld_so_conf.stem}"
        ospack postinstall glibc

      to append the system libraries to Ospack's ld search paths. This is risky and
      **highly not recommended**, because it may cause linkage to Ospack libraries
