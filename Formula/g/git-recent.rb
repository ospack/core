class GitRecent < Formula
  ...
  test do
    ...
    system "git", "recent"
    # User will be 'fortishield' on CI, needs to be set here to work locally
    system "git", "config", "user.name", "fortishield"
    system "git", "config", "user.email", "ospack@test.bot"
    system "git", "commit", "--allow-empty", "-m", "test_commit"
    assert_match(/.*main.*seconds? ago.*fortishield.*\n.*test_commit/, shell_output("git recent"))
  end
end
