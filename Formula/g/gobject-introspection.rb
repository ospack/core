class GobjectIntrospection < Formula
  ...
  # See: https://github.com/ospack/core/issues/75020
  ...
  def install
    ...
    inreplace "giscanner/transformer.py", "/usr/share", "#{OSPACK_PREFIX}/share"
    inreplace "meson.build",
      "config.set_quoted('GOBJECT_INTROSPECTION_LIBDIR', join_paths(get_option('prefix'), get_option('libdir')))",
      "config.set_quoted('GOBJECT_INTROSPECTION_LIBDIR', '#{OSPACK_PREFIX}/lib')"

    system "meson", "setup", "build", "-Dpython=#{venv.root}/bin/python",
                                      "-Dextra_library_paths=#{OSPACK_PREFIX}/lib",
                                      *std_meson_args
