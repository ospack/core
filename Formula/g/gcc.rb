class Gcc < Formula
  ...
  stable do
    ...
    patch do
      url "https://raw.githubusercontent.com/Ospack/formula-patches/f30c309442a60cfb926e780eae5d70571f8ab2cb/gcc/gcc-14.2.0-r2.diff"
  ...
  def install
    ...
    pkgversion = "Ospack GCC #{pkg_version} #{build.used_options*" "}".strip
    ...
    # We need to create `lib/gcc/xy` as a directory and not a symlink to avoid `ospack link` conflicts.
    (lib/"gcc"/version_suffix).install_symlink (lib/"gcc/current").children

    # Only the newest ospacked gcc should install gfortan libs as we can only have one.
  ...
  def post_install
    if OS.linux?
      ...
      libdir = OSPACK_PREFIX/"lib/gcc/current"
      specs.write specs_string + <<~EOS
        *cpp_unique_options:
        + -isysroot #{OSPACK_PREFIX}/nonexistent #{system_header_dirs.map { |p| "-idirafter #{p}" }.join(" ")}

        *link_libgcc:
        #{glibc_installed ? "-nostdlib -L#{libgcc} -L#{glibc.opt_lib}" : "+"} -L#{libdir} -L#{OSPACK_PREFIX}/lib

        *link:
        + --dynamic-linker #{OSPACK_PREFIX}/lib/ld.so -rpath #{libdir}

        *ospack_rpath:
        -rpath #{OSPACK_PREFIX}/lib

      EOS
      inreplace(specs, " %o ", "\\0%(ospack_rpath) ")
    end
  end
