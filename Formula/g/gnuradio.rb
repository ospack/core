class Gnuradio < Formula
  ...
  def install
    ...
    # Avoid references to the Ospack shims directory
    ...
    # Create a directory for Ospack to put .pth files pointing to GNU Radio
    ...
    (venv.site_packages/"ospack_gr_plugins.py").write <<~PYTHON
      import site
      site.addsitedir("#{plugin_pth_dir}")
    PYTHON

    pth_contents = "#{prefix/site_packages}\nimport ospack_gr_plugins\n"
    (venv.site_packages/"ospack-gnuradio.pth").write pth_contents

    # Patch the grc config to change the search directory for blocks
    inreplace etc/"gnuradio/conf.d/grc.conf", share.to_s, "#{OSPACK_PREFIX}/share"
