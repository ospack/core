class Janet < Formula
  ...
  def post_install
    ...
    resource("jpm").stage do
      ENV["PREFIX"] = prefix
      ENV["JANET_BINPATH"] = OSPACK_PREFIX/"bin"
      ENV["JANET_HEADERPATH"] = OSPACK_PREFIX/"include/janet"
      ENV["JANET_LIBPATH"] = OSPACK_PREFIX/"lib"
      ENV["JANET_MANPATH"] = OSPACK_PREFIX/"share/man/man1"
  ...
  def caveats
    <<~EOS
      ...
      - #{OSPACK_PREFIX}/lib/janet
      - #{OSPACK_PREFIX}/bin/jpm
      - #{OSPACK_PREFIX}/share/man/man1/jpm.1
    EOS
  end

  test do
    janet = bin/"janet"
    jpm = OSPACK_PREFIX/"bin/jpm"
