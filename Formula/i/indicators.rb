  ...
  test do
    (testpath/"test.cpp").write <<~CPP
      ...
      int main() {
        ...
        p.set_option(option::PrefixText{"Ospacking... "});
        ...
        auto job = [&p]() {
          while (true) {
            p.set_option(
                option::PrefixText{"Ospacking... " + std::to_string(p.current()) + "% "});
            if (p.current() + 1 > 100)
              p.set_option(option::PrefixText{"Ospacking... Done"});
            p.tick();
            if (p.is_completed()) {
              break;
            }
    ...
    assert_equal output.scan(/(?=Ospacking...)/).count, 100
    100.times do |n|
